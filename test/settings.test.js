...
test('defaults include a normal tab style and system theme', () => {
  const store = new SettingsStore(tmpFile());
  const settings = store.get();
  assert.equal(settings.tabStyle, 'normal');
...
test('set merges a partial patch', () => {
  const store = new SettingsStore(tmpFile());
  store.set({ tabStyle: 'compact' });
  assert.equal(store.get().tabStyle, 'compact');
...
test('settings persist across store instances', () => {
  ...
  assert.equal(reloaded.get().tabStyle, 'compact');
});
